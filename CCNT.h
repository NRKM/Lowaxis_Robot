...
#define	CNTM_TIMER								0x1140	// タイマー
#define	CNTM_COUNTER_ERROR						0x1141	// カウンターエラー
#define	CNTM_CARRY_BORROW						0x1142	// キャリー／ボロー

#define	CNTM_SAMPLING_STOP						0x1150	// サンプリング停止
#define	CNTM_SAMPLING_COUNT						0x1151	// 指定個数サンプリング完了
...
#define	CNT_ERR_SYS_RECOVERED_FROM_STANDBY		7		// スタンバイモードから復帰したため、CntResetDevice関数を実行してください
#define	CNT_ERR_INI_NOT_FOUND_SYS_FILE			8		// ccnt.sysファイルが見つからないため初期化できません
#define	CNT_ERR_INI_DLL_FILE_VERSION			9		// ccnt.dllファイルのバージョン情報が取得できないため初期化できません
#define	CNT_ERR_INI_SYS_FILE_VERSION			10		// ccnt.sysファイルのバージョン情報が取得できないため初期化できません
#define	CNT_ERR_INI_NO_MATCH_DRV_VERSION		11		// ccnt.dllとccnt.sysのバージョン情報が一致しないため初期化できません
// CNT
...
#define	CNT_ERR_SYS_OUTPUT_LOGIC				20301	// 制御出力信号の出力論理が設定可能範囲外です
#define	CNT_ERR_SYS_EVENT_TYPE					20302	// ハードウェアイベントの種類が設定可能範囲外です
#define	CNT_ERR_SYS_PULSE_WIDTH					20303	// ワンショットパルスのパルス幅の係数が設定可能範囲外です
#define	CNT_ERR_SYS_INPUT_LOGIC					20304	// 制御入力信号の入力論理が設定可能範囲外です
#define	CNT_ERR_SYS_OUTPUT_DATA					20305	// 出力データが設定可能範囲外です

// CNT Sampling
#define	CNT_ERR_SYS_START_SIGNAL				21000	// スタート信号が設定可能範囲外です
#define	CNT_ERR_SYS_STOP_SIGNAL					21001	// ストップ信号が設定可能範囲外です
#define	CNT_ERR_SYS_CLOCK_SIGNAL				21002	// クロック信号が設定可能範囲外です
#define	CNT_ERR_SYS_SIGNAL						21003	// 使用できる信号の範囲外です。 
#define	CNT_ERR_SYS_COUNTER_MODE				21004	// カウントモードが設定可能範囲外です
#define	CNT_ERR_SYS_START_TRG					21005	// スタート条件が設定可能範囲外です
#define	CNT_ERR_SYS_COUNT_START_MODE			21006	// カウント開始条件が設定可能範囲外です
#define	CNT_ERR_SYS_CLOCK_TRG					21007	// クロック条件が設定可能範囲外です
#define	CNT_ERR_SYS_STOP_TRG					21008	// ストップ条件が設定可能範囲外です
#define	CNT_ERR_SYS_CLOCK_VAL					21009	// クロック値が設定範囲外です。 
#define	CNT_ERR_SYS_CLOCK_UNIT					21010	// クロック値単位が設定範囲外です。
#define	CNT_ERR_SYS_STOP_NUM					21011	// サンプリング停止回数が設定範囲外です。
#define	CNT_ERR_SYS_COUNT_STOP_MODE				21012	// カウント停止条件が設定可能範囲外です

#define	CNT_ERR_SYS_BUFFER						21100	// バッファが大きすぎて確保できませんでした。
#define	CNT_ERR_SYS_LOCK_MEMORY					21101	// メモリがロックできませんでした。
#define	CNT_ERR_SYS_SEQUENCE					21102	// 実行手順エラー
